...
{{- define "pod.field.volumes" -}}
  ...
  {{- range $identifier, $persistenceValues := $persistenceItemsToProcess -}}
    ...
    {{- if eq (default "persistentVolumeClaim" $persistenceValues.type) "persistentVolumeClaim" -}}
      {{- $pvcName := (include "jellyfin.fullname" $rootContext) -}}
      ...
      {{- else -}}
        ...
        {{- if $persistenceValues.nameOverride -}}
          {{- if not (eq $persistenceValues.nameOverride "-") -}}
            {{- $pvcName = (printf "%s-%s" (include "jellyfin.fullname" $rootContext) $persistenceValues.nameOverride) -}}
          {{- end -}}
        {{- else -}}
          {{- $pvcName = (printf "%s-%s" (include "jellyfin.fullname" $rootContext) $identifier) -}}
        {{- end -}}
      {{- end -}}
